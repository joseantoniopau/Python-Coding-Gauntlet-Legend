...
### 5.1 The Final Fantasy VI model, and why it is right here

Researched: FF6 is 16×16 tiles — which this project already matches exactly —
and its **interiors are separate maps**. You step on a door tile and the map
changes to a furnished room: tables, chairs, stoves, armour on racks. Towns look
similar from outside; the character is inside. (The Spriters Resource FF6
tilesets; Tilting at Pixels' survey of the SNES-era Final Fantasy maps.)

That is the model. Copy the door-to-interior transition. Do not copy the
furniture pixel for pixel — everything here is generated at runtime.
